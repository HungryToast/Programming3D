...
namespace StarterAssets
{
	...
	public class FirstPersonController : MonoBehaviour
	{
		...
		{
			RaycastHit hit;
			GameObject pickupItemGameObject = null;
			if (_input.pickUp)
			{
				if (!itemInHand)
				{
					if (Physics.Raycast(_mainCamera.transform.position,_mainCamera.transform.TransformDirection(Vector3.forward), out hit, 3f))
					{
						if (hit.transform.gameObject.CompareTag("pickup"))
						{
							itemInHand = true;
							pickupItemGameObject = hit.transform.gameObject;
							pickupItemGameObject.GetComponent<PickUpController>().Pickup();
							hit.transform.position = itemPosition.position;
							hit.transform.parent = transform;
						}

						if (hit.transform.gameObject.CompareTag("button"))
						{
							hit.transform.gameObject.GetComponent<ButtonSpawnScript>().SpawnPickUpObject();
							_input.pickUp = false;
						}

						if (hit.transform.gameObject.CompareTag("door"))
						{
							DoorScript doorScript = hit.transform.gameObject.GetComponent<DoorScript>();
							if (!doorScript.doorState)
							{
								doorScript.OpenDoor();
							}
							else
				...
				else
				{
					foreach (Transform child in transform )
					{
						...
						{
							child.parent = null;
							child.GetComponent<PickUpController>().PutDown();
							break;
						}
					}
					itemInHand = false;
